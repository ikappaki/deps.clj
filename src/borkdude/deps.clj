...
(defn clojure-tools-jar-download-direct
  [source dest]
  (try
    (warn "\nDownloading with" ::direct-method... "(does not respect CLJ_JVM_OPTS)")
...
(def ^:private clojure-tools-info*
  (delay (let [version @version]
           {:ct-base-dir  "ClojureTools"
            ...
            :ct-extra-files-name ["exec.jar" "example-deps.edn" "tools.edn"]
...
(defn unzip [zip-file destination-dir]
  (let [{:keys [ct-base-dir ct-extra-files-name ct-jar-name]} @clojure-tools-info*
        zip-file (io/file zip-file)
        _ (.mkdirs (io/file destination-dir))
        ^ClassLoader x nil
        fs (FileSystems/newFileSystem (.toPath zip-file) x)]
    (doseq [f (into [ct-jar-name] ct-extra-files-name)]
...
(defn- clojure-tools-jar-download-java
  ...
  (let [dest-dir (.getCanonicalPath (io/file dest-zip-file ".."))
        ...
        success?* (atom true)]
    (warn "\nDownloading with" ::java-subprocess-method... "(requires Java11+)")
    (binding [*exit-fn* (fn
...
(defn clojure-tools-jar-download
  ...
  [deps-clj-config-dir java-cmd-line-args]
  (let [{:keys [ct-error-exit-code ct-url-str ct-zip-name]} @clojure-tools-info*
        dir (io/file deps-clj-config-dir)
        zip-file (io/file deps-clj-config-dir ct-zip-name)]
    (when-not (.exists zip-file)
      (warn "Downloading" ct-url-str "to" (str zip-file))
      (.mkdirs dir)
      (or (clojure-tools-jar-download-java ct-url-str (str zip-file) java-cmd-line-args)
          (clojure-tools-jar-download-direct ct-url-str zip-file)
  ...
  (let [opts (parse-args command-line-args)
        ...
        (or
         ...
         (binding [*out* *err*]
           ...
           (clojure-tools-jar-download tools-dir (vec (concat clj-jvm-opts
                                                              proxy-settings
                                                              ["-Xms256m"])))
           tools-jar))
        mode (:mode opts)
